...
class HonestPromise extends Promise {
	...
	static resolve(value) {
		return super.resolve.call(HonestPromise, value);
	}
	
	static reject(reason) {
		return super.reject.call(HonestPromise, reason);
	}
	
	static all(iterable) {
		if (notIterable(iterable)) return HonestPromise.reject(new TypeError('Expected argument to be an iterable object'));
		return super.all.call(HonestPromise, iterable);
	}
	
	static race(iterable) {
		if (notIterable(iterable)) return HonestPromise.reject(new TypeError('Expected argument to be an iterable object'));
		return super.race.call(HonestPromise, iterable);
	}
	...
	static after(ms, value) {
		if (HonestPromise.isPromise(value)) value.then(undefined, noop);
		return new HonestPromise((resolve) => {
