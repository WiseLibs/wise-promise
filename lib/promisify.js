'use strict';
const HonestPromise = require('./honest-promise');

HonestPromise.promisify = (fn, { deoptimize = false, multiArgs = false } = {}) => {
...
return function promisified(${generateArgumentList(maxArgCount).join(', ')}) {
	...
	return new Function(['HonestPromise', 'fn'], body)(HonestPromise, fn);
};

HonestPromise.nodeify = (fn) => {
	if (typeof fn !== 'function') throw new TypeError('Expected argument to be a function');
	return function nodeified() {
		...
		args.length -= 1;
		HonestPromise.resolve(fn.apply(this, args)).then((value) => {
			cb(null, value);
		}, (reason) => {
